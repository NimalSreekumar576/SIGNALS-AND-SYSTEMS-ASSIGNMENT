...
\author{Nimal Sreekumar\\EE23BTECH11044}
\date{}
...
$\Omega_{p1} = 0.443$, $\Omega_{p2} = 0.541$ and $\Omega_{s1} = 0.421$, $\Omega_{s2} = 0.567$
respectively.
...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.4895$ and $B = \Omega_{p2} - \Omega_{p1} = 0.098$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = -1.511\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = 1.473
\end{align}
...
we get $N\geq 4$ and $0.309 \leq \epsilon \leq 0.61$\\
\newpage
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/NimalSreekumar576/SIGNALS-AND-SYSTEMS-ASSIGNMENT/blob/main/filter_design/codes/plot1.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/plot1.png}
\caption{The Analog Low-Pass Frequency Response for $0.309  \leq \epsilon \leq 0.61$}
...
\begin{lstlisting}
https://github.com/NimalSreekumar576/SIGNALS-AND-SYSTEMS-ASSIGNMENT/blob/main/filter_design/codes/sk_gen
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/NimalSreekumar576/SIGNALS-AND-SYSTEMS-ASSIGNMENT/blob/main/filter_design/codes/pole_zero_plt.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
\begin{lstlisting}
https://github.com/NimalSreekumar576/SIGNALS-AND-SYSTEMS-ASSIGNMENT/blob/main/filter_design/codes/gain_LP.py
\end{lstlisting}
Substituting respective values in \eqref{eq:Gain_eq_LP} we get $G_{LP}=0.3125$
\begin{align}
     H_{a,LP}(s_L) &= \frac{0.3125}{\brak{s_L-s_1}\brak{s_L-s_2}\brak{s_L-s_3}\brak{s_L-s_4}}\\
     &= \frac{0.3125}{s_{L}^4 + 1.1068s_{L}^3 +1.6124s_{L}^2 + 0.9139s_{L} +0.33655}\label{eq:design}
\end{align}

\begin{lstlisting}
https://github.com/NimalSreekumar576/SIGNALS-AND-SYSTEMS-ASSIGNMENT/blob/main/filter_design/codes/plot2.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
\begin{lstlisting}
https://github.com/NimalSreekumar576/SIGNALS-AND-SYSTEMS-ASSIGNMENT/blob/main/filter_design/codes/gain_BP.py
\end{lstlisting}
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{2.88\times 10^{-5}s^4}{s^8 + 0.108s^7 + 0.973s^6 + 0.075s^5 + 0.351s^4 + 0.018s^3 + 0.055s^2 + 0.001s + 0.003} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{lstlisting}
https://github.com/NimalSreekumar576/SIGNALS-AND-SYSTEMS-ASSIGNMENT/blob/main/filter_design/codes/plot3.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{align}
    H_{d,BP}(z) &= \frac{G\brak{1 - 4z^{-2} + 6z^{-4} - 4z^{-6} + z^{-8}}}{2.584 - 12.404z^{-1} + 32.132z^{-2} - 53.316z^{-3} + 62.036z^{-4} - 51.004z^{-5} + 29.452z^{-6} - 10.892z^{-7} + 2.18z^{-8}}
\end{align}

where $G=2.88\times 10^{-5}$    
\begin{lstlisting}
https://github.com/NimalSreekumar576/SIGNALS-AND-SYSTEMS-ASSIGNMENT/blob/main/filter_design/codes/plot6.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
\begin{lstlisting}
https://github.com/NimalSreekumar576/SIGNALS-AND-SYSTEMS-ASSIGNMENT/blob/main/filter_design/codes/FIR_1.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{enumerate}
\begin{lstlisting}
https://github.com/NimalSreekumar576/SIGNALS-AND-SYSTEMS-ASSIGNMENT/blob/main/filter_design/codes/plot4.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\begin{align}
     \frac{\sin(\omega_{p2} n)}{n\pi} -\frac{\sin\brak{\omega_{p1}n}}{n\pi} &= 2\cos{\brak{\frac{\omega_{p2}n+\omega_{p1}n}{2}}}\sin{\brak{\frac{\omega_{p2}n-\omega_{p1}n}{2}}}\\
            &= \frac{2\cos{\brak{0.291n\pi}}\sin{\brak{0.050n\pi}}}{n\pi}
\end{align}
Multipying by window function we get :
\begin{align}
    h_{BP}\brak{n} = 
\begin{cases} 
   \frac{2\cos{\brak{0.291n\pi}}\sin{\brak{0.050n\pi}}}{n\pi},  & \text{for } -48\leq n \leq 48 \\
    0 &\text{otherwise}
\end{cases} \label{eq:h(n)desired_for_LPF}
\end{align}
\begin{lstlisting}
https://github.com/NimalSreekumar576/SIGNALS-AND-SYSTEMS-ASSIGNMENT/blob/main/filter_design/codes/plot5.py
\end{lstlisting}
\begin{figure}[H]
\centering
