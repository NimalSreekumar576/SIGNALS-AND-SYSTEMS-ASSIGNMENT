...
\begin{align}
    S(pq) &= \sum_{r=0}^{pq} x(r) \\
    S(pq) &= \sum_{r=0}^{pq} (x(0)+rd) \\
    &=(pq+1)x(0) +d\sum_{r=0}^{pq} r \\
    &=(pq+1)x(0) +d\left(\frac{pq(pq+1)}{2}\right)
\end{align}
...
\begin{tabular}{|c|c|c|}
    ...
    \hline
    $x(n)$ & $x(0) + nd$ & general term of the series \\
    \hline
    $S(n)$ & $ d\frac{n(n+1)}{2}$ & sum of n terms \\
    \hline
    $ X(z)$ & $\frac{z^{-1}}{pq(1-z^{-1})^2}$ & Z-transform of x(n)\\
    \hline
\end{tabular}\\
